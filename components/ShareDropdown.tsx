...
export default function ShareDropdown({ title, description, hashtags = 'fpark,주식', reportType, reportData }: ShareDropdownProps) {
  ...
  // ── 카카오톡 공유 ────────────────────────────────────────────────
  const handleKakao = () => {
    console.log('[Kakao] KAKAO_JS_KEY:', KAKAO_JS_KEY ? '설정됨' : '없음');
    ...
    try {
      const params = {
        ...
        content: {
          title: 'Finance Park - AI 종목진단',
          description: 'AI가 분석한 종목진단 리포트',
          imageUrl: 'https://fpark.com/og-image.png',
          link: {
            mobileWebUrl: 'https://fpark.com',
            webUrl: 'https://fpark.com',
          },
        },
        buttons: [
          {
      ...
      console.log('[Kakao] sendDefault params:', JSON.stringify(params, null, 2));
