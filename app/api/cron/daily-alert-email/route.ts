...
const DAILY_EMAIL_SYSTEM_INSTRUCTIONS = `당신은 국내 주식 시장 데이터를 있는 그대로 정리하는 정보 제공자입니다. 투자자문업자가 아니므로 매수/매도를 지시하거나 권유하지 마세요.
...
async function generateAiComment(
  ...
  const prompt = `다음은 오늘 투자자의 관심종목 등락 현황입니다:
${stockList}
  ...
  try {
    const message = await Promise.race([
      anthropic.messages.create({
        ...
        max_tokens: 2500,
        system: [{ type: 'text', text: DAILY_EMAIL_SYSTEM_INSTRUCTIONS, cache_control: { type: 'ephemeral' } }],
