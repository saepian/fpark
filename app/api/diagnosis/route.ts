...
// 반드시 일치.
// 2026-09-02: finalVerdict("AI 종합 진단") 스트리밍 지연 실측 — S-Oil 기준 finalVerdict가
// 46.2초/47.9초(96%) 지점에 도착, 사용자가 스크롤해서 기다려야 했다. 원인은 의존관계가 아니라
// 스키마 순서였다 — finalVerdict는 "앞선 필드들이 내린 판단"(background 개별/업종 판단,
// valuationNote 밸류에이션 부담, flowInsight 수급 대립, financialsNarrative 실적 추세) 4개에만
// 의존하는데, 그 사이에 무관한 9개 필드(historyNarrative·sectorNarrative·sectorTopPeersNarrative·
// disclosureNarrative·riskFactors·opportunityFactors·shortTermOutlook·midTermOutlook +
// institutionalFlow·foreignFlow)가 끼어 있어 순서상 맨 뒤로 밀려 있었다. finalVerdict 자체가
// 이 9개를 참고 금지("개별 사실·수치를 다시 인용하지 마세요")하므로 먼저 생성해도 품질에
// 영향이 없다고 판단해, 4개 의존 필드 바로 뒤로 앞당겼다(watchPoint 다음 → financialsNarrative →
// flowInsight/institutionalFlow/foreignFlow → finalVerdict, 이후 나머지는 기존 순서 유지).
