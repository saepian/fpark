...
- sectorNarrative·financialsNarrative는 mainAnalysis·riskFactors·shortTermOutlook·midTermOutlook과 내용이 겹치면 안 됩니다 — 업종/실적 이야기는 오직 이 두 필드에서만
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [priceResult, analysisResult, naverNewsResult, chartResult, sectorResult, financialsResult] = await Promise.allSettled([
...
분석 포인트:
1. 52주 위치와 PER/PBR으로 현재 주가 레벨에 대한 관찰 (과매수/과매도 구간 여부 관찰) — mainAnalysis에서 스치듯 한 번만, 핵심 근거로 쓰지 말 것
...
9. [업종 대비]에 peer 데이터가 있으면 sectorNarrative를, [실적 추이]에 데이터가 있으면 financialsNarrative를 채우세요 — 데이터가 없으면 해당 필드는 빈 문자열로 두고 mainAnalysis 등 다른 필드에서 억지로 대신 언급하지 마세요
...
위 데이터를 바탕으로 시스템 프롬프트에 제시된 JSON 스키마와 규칙에 따라 정리하세요.`;
    ...
    const diagnosisReportText = [
      ...
      finalResult.sectorNarrative, finalResult.financialsNarrative,
    ].filter(Boolean).join(' ');
