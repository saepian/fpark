...
위 데이터를 바탕으로 시스템 프롬프트에 제시된 JSON 스키마와 규칙에 따라 정리하세요.`;
    ...
    const diagnosisReportText = [
      finalResult.summary, ...finalResult.reasons, ...finalResult.opportunityFactors,
    ...
    // 온보딩 체크리스트: 첫 AI 진단 리포트 수신 완료 처리
    after(() => markOnboardingFlag(user.id, 'onboarding_report_viewed'));
