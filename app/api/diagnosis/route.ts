...
{
  "summary": "【450자 이내, 아래 구조로 작성 — 이 섹션만 읽어도 종목의 핵심 그림이 그려지도록 다른 섹션(수급·리스크·기회 요인)의 데이터를 적극 재활용해 풍부하게 작성】[1] 첫 문장: 현재 상태를 관찰형으로 — 예) '지금 삼성전자는 수익이 충분히 난 상태이며 외국인 자금 유출이 나타나고 있습니다.' [2] 밸류에이션 한 줄 코멘트: PER/PBR이 업종 평균 대비 어느 수준인지 관찰형으로 — 예) 'PER 44배 수준은 반도체 업종 평균 대비 높은 밸류에이션 구간으로 풀이됩니다.' [3] 이유 2~3가지를 일반인이 이해할 수 있는 쉬운 말로 — 예) '외국인 자금이 5일 연속 대규모로 빠져나가고 있는데, 이건 보통 큰손들이 차익 실현에 나설 때 흔히 보이는 패턴입니다.' [4] (외국인·기관과 개인의 자금 방향이 실제로 반대일 때만) 그 대립 구도를 명시 — 예) '외국인·기관 자금은 빠져나가는 반면 개인 자금은 반대로 유입되는 구도를 보입니다.' 방향이 같으면 이 문장은 생략. [5] (뉴스 논조와 실제 주가 흐름이 실제로 반대일 때만) 그 괴리를 강조 — 예) '~라는 긍정적 뉴스에도 불구하고 실제로는 하락한 점은 뉴스 외 다른 요인이 작용했을 가능성을 시사합니다.' 괴리가 없으면 이 문장은 생략. [6] 데이터 사실로 마무리 — 예) '현재 52주 고점·저점 대비 위치와 수급 방향은 이와 같이 관찰됩니다.' 결론을 유도하거나 '투자자들이 참고한다'는 식의 권유성 마무리 금지. 금지: 매수/매도/홀딩 같은 지시나 권유, '~하세요'/'~하는 게 좋습니다'/'권고'/'~전략이 현실적입니다' 같은 1인칭 조언 문장, 목표가·손절가 언급, 저항선·지지선·매물대 같은 기술적 분석 용어, ①②③ 번호 나열, 데이터 단순 나열, [4][5]에 해당 패턴이 없는데 억지로 만들어 넣기. 스타일: 편하게 설명하는 관찰형 어조를 쓰되, 위 예시들처럼 문장마다 종결 표현을 다양하게 바꾸고 같은 어미를 반복하지 마세요",
  "reasons": ["관찰된 근거 1 (수치 포함)", "관찰된 근거 2", "관찰된 근거 3"],
  "technicalAnalysis": ["기술적 관찰 1 (52주 고/저점 대비 위치 — 저항선·지지선 용어 없이 '몇 % 높은/낮은 수준'으로 서술)", "기술적 관찰 2 (거래량·모멘텀)"],
  ...
  "flowPercentage": 50,
  "shortTermOutlook": "단기(1개월) 관찰 — 52주 고점/저점 대비 위치 기준 조건부 서술 유지 가능 (예: '52주 고점 X원 부근 도달 시 이후 흐름이 관찰 포인트가 될 수 있습니다'), 목표가·저항선·지지선 언급 금지, 2문장",
  "midTermOutlook": "중기(3개월) 관찰 — 업황 변수와 52주 고점/저점 대비 위치 기준 시나리오 서술, 목표가·저항선·지지선 언급 금지, 2문장"
}
...
- "목표가", "손절가", "매수 추천", "매도 추천", "권고", "정당화", "저항선", "지지선", "매물대", "지지 시험" 단어를 사용하지 마세요
