...
- summarySections_background/newsInterpretation/historicalComparison/judgment 각 필드·riskFactors·opportunityFactors·historyNarrative·contributionNarrative·holdingPeriodNarrative·coMovementNarrative·shortTermOutlook·midTermOutlook은 서로 같은 사실을 반복 서술하지 마세요 — 각 필드는 서로 다른 내용을 담아야 합니다. 위 [종목별 개별 관찰](각 종목의 reason — 이미 "기업별 관찰 지표" 카드로 화면에 별도 표시됨)에 담긴 특정 종목의 구체적 사실(개별 뉴스 이슈, 특정 수치 등)도 그대로 옮겨 쓰지 마세요 — 이 필드들은 개별 종목 단위를 넘어서는 포트폴리오 레벨 관점을 각자 전담해야 합니다: newsInterpretation=그 뉴스가 왜 나왔는지의 배경, historyNarrative=직전 진단 대비 시간축 변화, coMovementNarrative=여러 종목에 걸친 동조화, contributionNarrative=오늘 손익 기여 구도, riskFactors/opportunityFactors=포트폴리오 구조적 위험·기회, judgment=위 판단들을 종합한 최종 스탠스(예외, 아래 참고). 여러 종목이 같은 이슈로 얽혀 있으면 종목별로 나열하지 말고 "왜 여러 종목이 동시에 영향받았는지"라는 상위 패턴으로 압축하세요.
