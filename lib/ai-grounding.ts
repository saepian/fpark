...
// 2026-07-27 신설: AI 리포트 4종(국내/해외 종목분석, 기업분석, 포트폴리오분석)이 휴장일
// (주말·공휴일)에도 마지막 거래일 데이터를 "오늘 마감/오늘 등락"인 것처럼 서술하던 문제의
// 재발 방지용. TEMPORAL_GROUNDING_INSTRUCTION이 "이미 지난 일을 미래형으로 쓰지 말라"를
// 다룬다면, 이건 "지난 거래일 데이터를 오늘 것처럼 쓰지 말라"는 같은 결의 문제다 — 원인도
// 같다(모델이 실제 데이터 기준일을 인지하지 못하고 사전지식/현재 시각으로 추측). 프롬프트의
// [거래일 상태] 블록(lib/market-day-context.ts의 buildMarketDayBlock)과 짝을 맞춰 쓴다.
export const MARKET_DAY_GROUNDING_INSTRUCTION =
  '[거래일 상태]가 휴장일이면 본문 어디에서도 "오늘 마감", "오늘 등락", "오늘 거래대금"처럼 ' +
  '서술하지 말고 반드시 "지난 거래일(N월N일) 마감 기준"이라고 명시하세요. 이때 시세·수급 ' +
  '데이터 자체는 실제 값 그대로 인용하되, 그 값이 언제 확정된 것인지만 정확히 밝히면 됩니다. ' +
  '휴장일 동안 나온 뉴스가 있다면 그것을 다음 거래일에 참고할 만한 사실로 언급하되, 이미 지난 ' +
  '휴장일 자체를 앞으로 열릴 것처럼 서술하지 마세요. [거래일 상태]가 거래일이면 이 지침은 ' +
  '무시하고 평소대로 서술하세요.';
